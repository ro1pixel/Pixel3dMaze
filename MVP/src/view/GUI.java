	...
	public GUI() {
		...
		mazeWindow.generateMazeSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent arg0) {
				...
				setChanged();
				notifyObservers("generate_maze " + mazeName + " " + gmw.getFloors() + " " + gmw.getHeight() + " "
						+ gmw.getWidth());
				setChanged();
		...
		mazeWindow.getHintSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent arg0) {
				...
				if (mazeWindow.getMaze() != null) {
					if (mazeWindow.getMazeDisplay().getCurrentPosition().getZ() > mazeWindow.getMazeDisplay().getGoal()
							.getZ())
						mazeWindow.displayInfoMessage("Hint", "The goal is BELOW you! (pageDown)");
					else if (mazeWindow.getMazeDisplay().getCurrentPosition().getZ() < mazeWindow.getMazeDisplay()
							.getGoal().getZ())
		...
		mazeWindow.saveMazeSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent arg0) {
				if (mazeWindow.getMaze() != null) {
					String fileName = mazeWindow.displayFileDialog(SWT.SAVE, "Save maze", new String[] { "*.maz" },
							"C:\\");
					if (fileName != null) {
						setChanged();
		...
		mazeWindow.propertiesSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent arg0) {
				...
				// mazeWindow.displayInfoMessage("Edit Properties", "the
				// Properties was edited succesfully");
			}
		...
		mazeWindow.importPropertiesSelectionListener(new SelectionListener() {
			...
			@Override
			public void widgetSelected(SelectionEvent arg0) {
				String fileName = mazeWindow.displayFileDialog(SWT.SAVE, "Save properties", new String[] { "*.xml" },
						"C:\\");
				if (fileName != null) {
					setChanged();
